...
int main(object me, string arg)
{
	...
	{
		return notify_fail("既然已经传承衣钵，如何舍得自杀？\n");
	}
	if(me->query("inherit_from"))
	{
		return notify_fail("你肩负着传承衣钵衣钵的重要使命，岂能自杀？\n");
	}

	if( me->query("combat_exp") > 200000 )
	{
		return notify_fail("山穷水尽疑无路，柳暗花明又一村。这位" + RANK_D->query_respect(me) +"不要意气用事，还是先冷静一下再说吧！提示：你的经验超出范围。\n");
	}
