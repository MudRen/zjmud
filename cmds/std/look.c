...
string get_inv_act(object me, object obj)
{
	...
	if(present(obj,me))
	{
		...
		if(objectp(present("chuwu dai",me)))
			look_msg3 += "存入 "+present("chuwu dai",me)->query("name")+":baocun "+file_name(obj)+ZJSEP;
...
//	}
	...
	if(userp(obj))
	{
        ...
        if(obj->is_character())
	{
		str += HIR"偷窃"NOR":steal " + obj->query("id")+ZJSEP;
	}
	...
	{
		str += "超级禁言:blk " + file_name(obj)+ZJSEP;
		str += "踢出游戏:kickout " + file_name(obj)+ZJSEP;
	}
