...
//		return notify_fail("华山论剑起码要有100内功激发，你还是把功夫练好再来吧。\n");
...
/*
				for(i=0;i<sizeof(total);i++)
				{
					if(i==0)
					{
						...
						{
							ob->add("yuanbao",1500);
							tell_object(ob,"你在本次华山论剑中获得官方奖励1500元宝。\n");
							log_ufile(ob,"lunjian","论剑冠军奖励元宝1500。\n");
						}
					}
					else if(i>0 && i<5)
					{
						...
						{
							ob->add("yuanbao",800);
							tell_object(ob,"你在本次华山论剑中获得官方奖励800元宝。\n");
							log_ufile(ob,"lunjian","论剑5强奖励元宝800。\n");
						}
					}
					else
					{
						if(ob = find_player(total[i]))
						{
							...
							tell_object(ob,"你在本次华山论剑中获得官方奖励20000潜能\n");
							log_ufile(ob,"lunjian","论剑参与奖励潜能20000。\n");
						}
					}
				}
*/
		...
		else if(sizeof(inv)==2)
		{
			if(time() - stime > OUT_TIME)
			{
				...
				if(hp1>hp2)
				{
					...
					log_ufile(inv[1],"lunjian","胜利晋级，积分增加1。\n");
					check_win(inv[1]);
					check_out(inv[0]);
				}
			}
