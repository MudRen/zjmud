...
(function (Date, undefined) {
	...
	Ansi_Up.prototype.process_chunk = function (text, options, markup) {
		...
		if(text.indexOf('u:cmds:')==0)
		{
			text = text.replace('u:cmds:','');
			var ss = text.split(']');
			if(ss[1]!='')
				text = " <input type=\"button\" class=\"ucmds\" onclick=\"cmds(this)\" value=\""+ss[1]+"\" id=\""+ss[0]+"\">";
			else
			{
				tmpurl = ss[0];
				return '';
		...
		if(tmpurl!='')
		{
				...
				text = " <input type=\"button\" class=\"ucmds\" onclick=\"cmds(this)\" value=\""+text+"\" id=\""+tmpurl+"\">";
				tmpurl = '';
				return text;
		}
